...
async def generate_report(execution_output: ExecutionOutputSchema):
    ...
    try:
        ...
        # --- Integration Merge ---
        # Add the ML risk data back into the report after Module 9 
        # (Module 9 only knows about rule-based violations)
        report["risk_summary"] = raw.get("risk_summary")
        report["risk_data"] = raw.get("risk_data")
        report["explanations"] = raw.get("explanations")
